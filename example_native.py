...
filename = "RD_" + args.fl + ".txt"
filename = os.path.join(input_path, subreddit_number, args.fl)
...
_, total, pred_error = predict(events=events,
                               obs_time=obs_time,
                               pred_time=pred_time,
                               p_max=None,
                               params=params,
...
sys.stdout.flush()
print("Predicted number of retweets at hour %s: %i" %
      (pred_time, total_pred))
sys.stdout.flush()
print("Prediction error (absolute): %.0f" % pred_error)
sys.stdout.flush()

filename = "RD_" + args.fl + ".json"
